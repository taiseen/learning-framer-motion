...
const MountingAnimation = () => {
    ...
    return (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 mx-auto w-1/2 p-4 bg-slate-500/50 rounded flex flex-col gap-4">

            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        className="p-4 w-1/2 rounded bg-green-600 cursor-pointer space-y-3"
                    ...
                    >
                        <p className="p-2 border rounded">local state persist - <span className="text-xl font-semibold">{internalState}</span></p>

                        <p
                            className="p-2 border w-fit cursor-pointer rounded"
                            onClick={(e) => {
                                e.stopPropagation();
            ...
            <p
                className="p-2 rounded cursor-pointer border border-slate-800 w-fit text-xl"
                onClick={() => setIsOpen(prev => !prev)}
            >
                Click to -
